...
import { LightboxVideo } from "@/components/ui/lightbox-video";
...
const projects = [
  ...
  { title: "个人专属工作台", en: "Personal AI Workspace", desc: "用自然语言统一调度待办、文件搜索与日程协同的多智能体工作台。", tech: "Agent · Prompt · Tool Calling", image: `${ASSET_PREFIX}/projects/workspace.jpg`, video: `${ASSET_PREFIX}/projects/workspace-demo.mp4` },
...
export default function HomePage(): ReactNode {
  return (
    <main className="portfolio-shell">
      ...
      <section className="projects-section" id="projects"><div className="section-heading"><div className="section-kicker">02 / SELECTED WORK</div><h2>我正在构建的东西</h2><p>每个项目都来自一个具体问题，并以“内部测试”数据持续验证。</p></div><div className="project-grid">{projects.map((p, i) => <article className="project-card-custom" key={p.title}><div className="project-image">{p.video ? <LightboxVideo src={p.video} poster={p.image} title={`${p.title} 演示视频`} className="project-video" /> : <LightboxImage src={p.image} alt={p.title} className="h-full w-full object-cover" />}<span>0{i + 1}</span></div><div className="project-body"><p className="project-tech">{p.tech}</p><h3>{p.title}</h3><p className="project-en">{p.en}</p><p>{p.desc}</p><a href="#contact">了解项目 <span>↗</span></a></div></article>)}</div></section>
